...
class NoAsAServiceTwoMakeError
{
    public static function call(?NoAsAServiceTwoContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NoAsAServiceTwoContext([], null);
        }
        $op = $ctx->op ?? new NoAsAServiceTwoOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NoAsAServiceTwoResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NoAsAServiceTwoError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new NoAsAServiceTwoError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof NoAsAServiceTwoError) {
