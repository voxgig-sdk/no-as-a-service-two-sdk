...
//    only - a typed NoAsAServiceTwoContext there would make every ctx
...
final class NoAsAServiceTwoOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NoAsAServiceTwoContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NoAsAServiceTwoSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NoAsAServiceTwoResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NoAsAServiceTwoError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NoAsAServiceTwoResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NoAsAServiceTwoError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NoAsAServiceTwoError(is_string($code) ? $code : '', $msg);
    }
