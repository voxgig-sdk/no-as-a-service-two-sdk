...
// NoAsAServiceTwo SDK feature test
//
...
class FtCtrl extends NoAsAServiceTwoControl
{
...
class FtClient
{
    ...
    private NoAsAServiceTwoUtility $utility;

    public function __construct(NoAsAServiceTwoUtility $utility, array $options)
    {
    ...
    public function get_utility(): NoAsAServiceTwoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NoAsAServiceTwoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NoAsAServiceTwoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NoAsAServiceTwoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NoAsAServiceTwoConfig::make_feature($name);
    ...
    private static function buildUrl(NoAsAServiceTwoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NoAsAServiceTwoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NoAsAServiceTwoResponse($fetched);
        }
    ...
    private function fail(NoAsAServiceTwoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NoAsAServiceTwoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
