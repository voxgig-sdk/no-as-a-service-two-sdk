...
// NoAsAServiceTwo SDK RejectionReason entity
...
class RejectionReasonEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RejectionReasonEntity($this->_client, $opts);
    }

    /**
     * @param RejectionReason|array $args RejectionReason data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NoAsAServiceTwoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RejectionReason|array The current RejectionReason data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RejectionReason fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NoAsAServiceTwoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RejectionReason fields).
     */
     ...
     *
     * @param RejectionReasonLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RejectionReasonLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RejectionReason|array The loaded RejectionReason as an assoc-array at the
     *   SDK boundary; throws NoAsAServiceTwoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NoAsAServiceTwoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
