...
// NoAsAServiceTwo SDK retry feature
...
class NoAsAServiceTwoRetryFeature extends NoAsAServiceTwoBaseFeature
{
    ...
    public function init(NoAsAServiceTwoContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NoAsAServiceTwoContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NoAsAServiceTwoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NoAsAServiceTwoContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
