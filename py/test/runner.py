# NoAsAServiceTwo SDK test runner
...
class NoAsAServiceTwoTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NoAsAServiceTwoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NoAsAServiceTwoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NoAsAServiceTwoTestRunner.getenv("NO_AS_A_SERVICE_TWO_TEST_LIVE")
        override = NoAsAServiceTwoTestRunner.getenv("NO_AS_A_SERVICE_TWO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NoAsAServiceTwoTestRunner.getenv(key)
        ...
        explain = NoAsAServiceTwoTestRunner.getenv("NO_AS_A_SERVICE_TWO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NO_AS_A_SERVICE_TWO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NoAsAServiceTwoTestRunner._test_control is not None:
            return NoAsAServiceTwoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NoAsAServiceTwoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NoAsAServiceTwoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NoAsAServiceTwoTestRunner.load_test_control()
...
def load_env_local():
    NoAsAServiceTwoTestRunner.load_env_local()


def env_override(m):
    return NoAsAServiceTwoTestRunner.env_override(m)


def entity_data(v):
    return NoAsAServiceTwoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NoAsAServiceTwoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NoAsAServiceTwoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NoAsAServiceTwoTestRunner.load_test_control()


def live_client_options():
    return NoAsAServiceTwoTestRunner.live_client_options()


def live_delay_ms():
    return NoAsAServiceTwoTestRunner.live_delay_ms()
