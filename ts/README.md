...
`client.RejectionReason()` — each with a small set of operations (`load`)
...
const rejectionreason = await client.RejectionReason().load()
// rejectionreason is a bare entity populated with mock response data
console.log(rejectionreason)
```
...
| `tester(testopts?, sdkopts?)` | `NoAsAServiceTwoSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `NoAsAServiceTwoSDK.test(testopts?, sdkopts?)` | `NoAsAServiceTwoSDK` | Create a test-mode client. |
...
| `client` | `client(): NoAsAServiceTwoSDK` | Return the parent SDK client. |
...
```ts
const rejection_reason = await client.RejectionReason().load()
```
...
no-as-a-service-two/
├── src/
│   ├── NoAsAServiceTwoSDK.ts        # Main SDK class
