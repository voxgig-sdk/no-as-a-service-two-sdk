...
//     (\`new NoAsAServiceTwoSDK(...)\` / \`NoAsAServiceTwoSDK.test(...)\` become
//     \`NoAsAServiceTwoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NoAsAServiceTwoSDK'
...
const TEST_SEED = {"entity":{"rejection_reason":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NoAsAServiceTwoSDK(...)\` and any
// \`NoAsAServiceTwoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NoAsAServiceTwoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NoAsAServiceTwoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
