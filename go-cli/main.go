// Command NoAsAServiceTwo-cli is a CLI + REPL for the NoAsAServiceTwo SDK,
...
const prompt = "no-as-a-service-two"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "rejection_reason"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NO_AS_A_SERVICE_TWO_APIKEY (api key), NO_AS_A_SERVICE_TWO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NO_AS_A_SERVICE_TWO_APIKEY carries the API key and
	// NO_AS_A_SERVICE_TWO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NO_AS_A_SERVICE_TWO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NO_AS_A_SERVICE_TWO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNoAsAServiceTwoSDK(opts)
