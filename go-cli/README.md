# no-as-a-service-two-cli

boru-driven command-line client **and** interactive REPL for the NoAsAServiceTwo
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/no-as-a-service-two-cli)
make build

# 2. See usage (words, entities, env vars)
./no-as-a-service-two-cli --help

# 3. Provide credentials once, via the environment
export NO_AS_A_SERVICE_TWO_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./no-as-a-service-two-cli load 1 rejection_reason            # {id:1} shorthand
./no-as-a-service-two-cli load '{id:1}' rejection_reason       # explicit match map

# 5. Override the API base URL for a single call
NO_AS_A_SERVICE_TWO_BASE=https://api.example.com ./no-as-a-service-two-cli load 1 rejection_reason

# 6. No arguments -> interactive REPL
./no-as-a-service-two-cli
no-as-a-service-two> load 1 rejection_reason
no-as-a-service-two> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/no-as-a-service-two-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/no-as-a-service-two-cli load 1 rejection_reason
   ```
...
```sh
./no-as-a-service-two-cli load 1 rejection_reason          # scalar shorthand for {id:1}
./no-as-a-service-two-cli load '{id:1}' rejection_reason     # explicit match map
```
...
./no-as-a-service-two-cli load 1 rejection_reason
```
...
Run with no arguments to open a REPL (prompt `no-as-a-service-two>`). Each line is
evaluated as its own boru expression:

```text
$ ./no-as-a-service-two-cli
no-as-a-service-two> load 1 rejection_reason
no-as-a-service-two> /help
no-as-a-service-two> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `rejection_reason`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/no-as-a-service-two-cli`. |
...
(`./no-as-a-service-two-cli <expr>`) and interactively (the REPL), and expressions compose the
