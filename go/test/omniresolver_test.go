...
import (
	...
	omni "github.com/voxgig-sdk/no-as-a-service-two-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NoAsAServiceTwoError); is {
				out := map[string]any{
					"name":    "NoAsAServiceTwoError",
...
func omniCtx(arg any, client *sdk.NoAsAServiceTwoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NoAsAServiceTwoSDK); is {
			client = live
