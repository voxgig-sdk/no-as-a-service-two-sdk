...
class RejectionReasonEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NO_AS_A_SERVICE_TWO_TEST_REJECTION_REASON_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    rejection_reason_ref01_data_dt0_loaded = rejection_reason_ref01_ent.load(rejection_reason_ref01_match_dt0, nil)
...
def rejection_reason_basic_setup(extra)
  ...
  entid_env_raw = ENV["NO_AS_A_SERVICE_TWO_TEST_REJECTION_REASON_ENTID"]
