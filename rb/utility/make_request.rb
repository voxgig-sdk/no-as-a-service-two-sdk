...
module NoAsAServiceTwoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NoAsAServiceTwoResponse.new({})
    ...
    if fetch_err
      ...
      response = NoAsAServiceTwoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NoAsAServiceTwoResponse.new(fetched)
    else
