...
require_relative 'NoAsAServiceTwo_types'
...
class NoAsAServiceTwoSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NoAsAServiceTwoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NoAsAServiceTwoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NoAsAServiceTwoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NoAsAServiceTwoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NoAsAServiceTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NoAsAServiceTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NoAsAServiceTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue NoAsAServiceTwoError => err
    ...
    if fetched.is_a?(Hash)
      status = NoAsAServiceTwoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NoAsAServiceTwoError.new(
        "graphql_error", "NoAsAServiceTwoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NoAsAServiceTwoSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
