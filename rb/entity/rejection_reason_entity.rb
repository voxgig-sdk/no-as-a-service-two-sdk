...
class RejectionReasonEntity
  ...
  def make
    opts = @_entopts.dup
    RejectionReasonEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NoAsAServiceTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = NoAsAServiceTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NoAsAServiceTwoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
